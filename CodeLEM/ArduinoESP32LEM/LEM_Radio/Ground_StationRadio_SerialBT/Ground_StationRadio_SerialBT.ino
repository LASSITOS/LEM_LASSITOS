...
void loop() {
  ...
  if (RTCM_in.available()) { 
    while(RTCM_in.available())  {   // If anything comes in Serial (USB),
      // Radio.write(RTCM_in.read());   // read it and send it out to Radio
      
            buff_byte=RTCM_in.read() ; // read it and save to vatriable
      Radio.write(buff_byte);   
      // Serial.write(buff_byte);   
      
      RTCM_inbits_buff++;  // count incoming bytes
  ...
  // if (RTCM_inbits_buff>5000) {      // If anything comes in Serial is passed to NMEA parser
    if (millis()-RTCM_timer>dTime_RTCMmessage) {      // If anything comes in Serial is passed to NMEA parser

      RTCM_timer=millis();
      RTCM_rate=(float)RTCM_inbits_buff/dTime_RTCMmessage;
      RTCM_inbits+=RTCM_inbits_buff/1000;
      RTCM_inbits_buff=0;
      
      
      Serial.print("Total kB received RTCM port:");
      Serial.print(RTCM_inbits);
      Serial.print("RTCM data rate:");
      Serial.print(RTCM_rate);
      Serial.println(" kB/s");


      // SerialBT.print("Tot. Base RTCM: ");
      // SerialBT.print(RTCM_inbits);
      // SerialBT.println(" kB");
      SerialBT.print("RTCM_rate:");
      SerialBT.print(RTCM_rate);
      SerialBT.println(" kB/s");
    }
