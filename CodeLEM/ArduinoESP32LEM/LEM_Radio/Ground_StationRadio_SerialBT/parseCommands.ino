...
void parse( String rxValue){     //%toCheck
  ...
  if (rxValue.indexOf("START") != -1) {
    ...
    if(!validMsg){
	  ...
	  Serial.println("Setting Gain ADC! ");
    int index = rxValue.indexOf(":");\
    int index2 = rxValue.indexOf(":",index+1);
    int valout[2];
    if (index !=-1 and index2 !=-1){
      valout [0]=rxValue.substring(index+1,index2).toInt();
      makeLEMNMEAMSG_values("SETGAINADC", 10, valout, 1);

    } else {
      sprintf(txString,"GAINADC can not be parsed from string '%s''",rxValue);
      Serial.println(txString);
    }
      ...
      if (index !=-1 and index2 !=-1){
        ...
        makeLEMNMEAMSG_values("SETGAIN2", 7, valout, 1);
