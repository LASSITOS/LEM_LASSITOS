...
int16_t SDselect (void)                   /* 1:OK, 0:Timeout */
{
...
DRESULT disk_write_multiple_init( uint32_t sector )
{
    DSTATUS s;
    s = disk_status(0);
    if (s & STA_NOINIT){
...
DRESULT  disk_write_multiple(const uint8_t *buff){
    if (!xmit_datablock(buff, 0xFC)){
        return RES_ERROR;
    }
    return RES_OK;
...
DRESULT disk_Write_multiple_stop(void){
    if (!xmit_datablock(0, 0xFD)){                              /* STOP_TRAN token */
        return RES_ERROR;
    }
    return RES_OK;
